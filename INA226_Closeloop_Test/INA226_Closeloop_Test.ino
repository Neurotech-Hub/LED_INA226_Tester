...
// Configuration
float shuntResistance = 0.04195;  // 39mΩ shunt
float maxCurrent_mA= 1500;           // 1.5A max current
float current = 0.0;
...
void loop() {
  if (Serial.available())
    ...
    Serial.println(ALERT_CONVERSION_READY);

    while (!alertFlag & ALERT_CONVERSION_READY=){
      if (targetCurrent_mA > 0)
      {
          // startTime = micros();
...
bool initializeINA226() {
  ...
  // Set calibration
  int result = ina226.setMaxCurrentShunt(maxCurrent_mA, shuntResistance, true);
  if (result != INA226_ERR_NONE) {
    Serial.println("Calibration failed with error: 0x" + String(result, HEX));
    return false;
  }
